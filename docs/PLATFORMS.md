...
| **Workable search** (`jobs.workable.com`) | Workable's own search across every employer it hosts, rather than a board, and it needs no key. **Twenty results a page behind an opaque `nextPageToken`**, so the walk is strictly sequential and cannot be parallelised; `limit=100` is a **400**, and `pageSize`, `size`, `per_page` and `page_size` are all accepted and all ignored. Fifteen pages is the cap, 300 postings per title, and a scan says so when it bites rather than quietly returning the first 300. It carries the full advert, so these roles need no `enrich` pass. `{keyword}` and `{country}` are both narrowed at the query: "software engineer" worldwide is 4,220 postings over 211 pages, and the same search in the United Kingdom is 322. Deliberately a different host from `apply.workable.com`, so a block on one does not silently take out the other. **Not a faster one**, though: it ran at the unmeasured 3.0/s default, and every scan from 7 to 17 September 2026 ended with it answering 429 with a `Retry-After` of 11 to 24 hours, leaving 39 to 61 searches UNKNOWN. A scan was sending it roughly 1,100 to 1,800 requests, most of them the recently-posted sweep (`?day_range=7`, 21,062 postings, 1,054 pages) walked to exhaustion. It is the same company's infrastructure as the boards, whose limit is a long-window quota, so it now gets both controls: **0.7/s**, the boards' rate, and **150 requests per run** (`fetch.HOST_REQUEST_BUDGET`), under the 176th request at which `apply.workable.com` has been seen to start refusing. Both are ceilings, not measurements of this host, and should not be probed for. The sweep is held to **30 pages** so the keyword searches keep most of the budget. A search stopped part way is marked cut off, and a source the spent budget turned away is reported as UNKNOWN, never as an empty result. A 429 with a long `Retry-After` still blocks the host for the rest of the run after one refusal, and the block is remembered between runs. Scored as an aggregator, so the employer's own board wins the row: 36% of what it finds is an employer already on the list. |
